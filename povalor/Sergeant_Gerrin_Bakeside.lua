...
function event_say(e)
	if e.message:findi("hail") then
		e.self:Say("Greetings, Soldier.  I'm sorry, if you've come for weapons, what you see here is only for use by the army of Marr. Right now we are not accepting new recruits into [" .. eq.say_link("Ducee Lealson") .. ".]")
	elseif e.message:findi("Ducee Lealson") then	
		e.self:Say("We are Ducee Lealson, Ducee Lealson is Loyalty!  Fyking, Fellon, Rydric, and myself are renowned through these areas for our skill in battle.  Each man in Ducee Lealson is not only willing to die for his compatriots, but he considers it an [" .. eq.say_link("honor") .. ".]")

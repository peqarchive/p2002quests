...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("What is it you want! Can't you see I am gathering [" .. eq.say_link("reagents") .. "] for my [" .. eq.say_link("master") .. "]? If I don't get them for him quickly he might punish me.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 20642})) then
		e.self:Say("I see you serve my master as well now. I assume you are here to take the reagents from me which I have gathered for him? I don't know if I like the idea of you gaining the credit for my hard work here. I might consider giving you these reagents, if only you do a small [" .. eq.say_link("task") .. "] for me.");
		e.other:QuestReward(e.self,0,0,0,0,20643,500);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 1320})) then -- flowing black robe turn in

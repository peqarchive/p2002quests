...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 20876})) then -- A love letter (Sealed)
		...
		e.other:QuestReward(e.self,0,0,0,0,20877,1000); -- Albino Rattlesnake Eggs
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20878})) then -- Symbol of Achievement
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20881,item2 = 20880,item3 = 10151,item4 = 5408})) then -- A burnt sash, A Gift, Adamantite Band and Electrum-Bladed Wakizashi
		...
		eq.spawn2(15183,0,0,-800,-2712,15.6,276); -- Vance Bearstalker
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20882})) then -- A Poachers Head
		...
		e.other:Faction(304,25,0); -- soldier of tunare
		e.other:Faction(63,-25,0); -- crushbone orcs
		e.other:QuestReward(e.self,0,0,0,0,15691,10000); -- Spell: Call of Flame
	end

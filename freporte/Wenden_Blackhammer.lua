function event_say(e)
	if(e.other:Class() == "Warrior") then
		...
		elseif(e.message:findi("golems")) then
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20669})) then
		e.self:Say("Where in the world did you drag that up?!' He peers closely at the severely damaged dragon head hilt. 'This metal is the same material that some mages use to make powerful [" .. eq.say_link("golems") .. "] and this other metal looks like [" .. eq.say_link("rejesiam") .. "] ore. If you can find some of those materials, I can fix this hilt up easily.");

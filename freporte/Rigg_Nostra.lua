function event_say(e)
	if(e.message:findi("crate of pickheads")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 19917})) then
		e.self:Say("Excellent I've been waiting for the Coalition to send me some assistance. I have an associate waiting for some supplies that have not yet been collected. I need you to acquire a [" .. eq.say_link("crate of pickheads") .. "] and a [" .. eq.say_link("bundle of hardwood shafts") .. "] and bring them to me so that I may prepare them for delivery to my associate.");

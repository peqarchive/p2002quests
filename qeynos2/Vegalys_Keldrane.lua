...
function event_say(e)
	...
	elseif(e.message:findi("careless")) then
		e.self:Say("The sudden influx of scoundrels and loathsome individuals was our first tip off. They required a rather enormous number of supplies, materials and manpower to construct the ornate altars, craft and enchant their runed branding irons and supply their armies with the weapons needed for the cleanup work afterward. They needed to rely upon [" .. eq.say_link("outside help") .. "].");
	elseif(e.message:findi("outside help")) then
		e.self:Say("They mostly chose to hire bandits and brigands who are foolishly more concerned with immediate profits than the long-term effects of their actions. But once they knew we were on to them they even started hiring some of the more 'civil' gnolls, if there is such a thing. Gnolls tend to be easy to push around, stupid, and they work very cheaply. They knew their time was short and they were desperate. Choosing to work with untrustworthy, lazy scoundrels caused the whole thing to begin to [" .. eq.say_link("unravel") .. "], praise Rodcet!");
	elseif(e.message:findi("unravel")) then
		e.self:Say("Their workers were careless. Aside from noticing the unsavory element in our city, our citizens began to run across many of the strange items and materials that were being trafficked into our city en masse. It was at that point that the citizens began to [" .. eq.say_link("complain") .. "] quite loudly about all of this.");
	elseif(e.message:findi("complain")) then
		e.self:Say("Obviously, they questioned the meaning behind these events and rightfully so. They were frightened much as they are now. I admit, I was remiss in my duties for not noticing it sooner. One of the biggest complainers during this time was a man by the name of [" .. eq.say_link("Earron Huntlan") .. "], proprieter of the famed Lion's Mane tavern. Though he is just a simple merchant, he did us a great service.");
	elseif(e.message:findi("Earron Huntlan")) then
		e.self:Say("Earron is quite a character. He gets all kinds of travelers stopping by his inn for a drink or whatever other business. His patrons started telling stories of being mugged and harassed by these loathsome individuals in the very streets of our fair city. Of course, he was concerned. So he became a kind of advocate for this cause, ringing the [" .. eq.say_link("alarm") .. "] bell as it were. I admit, at first I did not believe the conspiracy. I simply could not believe it was all happening under our nose.");

function event_say(e)
	if(e.other:GetFaction(e.self) == 1) then --Ally
		if(e.message:findi("hail")) then
			e.self:Say("Welcome, young mortal. Word of your deeds has preceded your arrival. Your intentions seem honorable, young one, however my trust is not an easy thing to win these days. Like you, there has been another that came before me with the trust of the dragon kin, and trust them too I did, but alas that proved to be destructive. I note your deeds to this shrine young one, but should you wish my counsel, you must [" .. eq.say_link("I wish to prove my worth",false,"prove your worth") .. "] beyond doubt. Perhaps a small challenge shall be of interest to you?");
...
function event_trade(e)
	...
	if(e.other:GetFaction(e.self) == 1) then --Ally
		if(item_lib.check_turn_in(e.self, e.trade, {item1 = 30516} )) then --King Tormax's Head
			...
			e.self:Say("I look upon this face, and even though he is dead, the hatred burns on. My grief and anger are unchanged even in the face of ultimate fate. His grandfathers act was so despicable and all-encompassing that it instilled an... immortal loathing. Nevertheless, you have completed the task I set before you and, as such, you deserve my gratitude. Please accept this [" .. eq.say_link("treasure") .. "].");
	...
	--omitting this due to return script already shown below
	--[[else  
		local traded_item = e.trade[" .. eq.say_link("'item1'") .. "]:GetID();

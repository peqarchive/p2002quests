...
function event_say(e)
	...
	if(qglobals["Llara"] ~= nil) then
		...
		elseif(e.message:findi("unusual")) then
			e.self:Say("When we were growing up we used to venture outside the city gates. Some times we would encounter a wandering ghoul or an undead spirit of some sort. He was always able to quickly dismiss the undead being with very little effort. I [" .. eq.say_link("learn",false,"learned") .. "] many things growing up with Amstaf.");
		...
		elseif(e.message:findi("ruathey")) then
			...
			e.other:SummonItem(2417); -- Note to Ruathey
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 2414})) then -- A Pendant
		e.self:Say("Oh my, this belongs to Amstaf! Where did you get it?");
		eq.set_global("Llara","ghoul",0,"D30");
		e.other:QuestReward(e.self,0,0,0,0,0,500); -- Exp and Flag
	end

function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Pleased to meet you %s. I am Vansted Rohenga High Priest of Karana. I have spent all of my years studying the ways of the Cleric in our wonderful Temple of Thunder. We receive many new recruits every day and it is my duty to place them in my [" .. eq.say_link("training program") .. "].",e.other:GetName()));
	elseif(e.message:findi("training program")) then
		e.self:Say("Well all young clerics of Karana are brought up from their first day of training to know and respect their practice. I have developed a number of exercises that will enable a young cleric to make a set of their own armor by means of collecting certain items and making [" .. eq.say_link("enchanted armor materials") .. "] out of them.");
	elseif(e.message:findi("enchanted armor materials")) then
		e.self:Say("Recently we have discovered that combing certain items in certain quantities using a magical assembly kit can forge together an armor material. These armor materials are used with specific armor molds to create a piece of enchanted Stormhealer Armor. If you would like to [" .. eq.say_link("begin forging armor materials") .. "] I will present you with your Stormhealers Assembly Kit.");
	elseif(e.message:findi("begin forging armor materials")) then
		e.self:Say(string.format("Excellent %s, please take this Stormhealers Assembly Kit. It is in this kit that you will combine certain items together into materials necessary for crafting your armor. I can present you with the material recipes and armor molds for Stormhealer [" .. eq.say_link("Helms") .. "], [" .. eq.say_link("Bracers") .. "], [" .. eq.say_link("Armguards") .. "], [" .. eq.say_link("Boots") .. "], [" .. eq.say_link("Greaves") .. "], [" .. eq.say_link("Gauntlets") .. "] and [" .. eq.say_link("Breastplates") .. "]. When you are ready to gather the items for an armor piece simply tell me what piece you want to craft. Once you have created a specific material take it to a forge along with the applicable mold to create your armor piece.",e.other:GetName()));

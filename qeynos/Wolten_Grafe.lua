function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hail, noble " .. e.other:GetName() .. ". Remember to spread the words of Karana throughout the faraway lands on which you shall tread. We of the Temple of Thunder are looking for new members - good people who [" .. eq.say_link("I will join the cause",false,"wish to join our cause") .. "]. Let the call go out!");
	elseif(e.message:findi("join the cause")) then
		e.self:Say("That IS good news! If you walk the righteous path of the Rainkeeper, Karana, then you are truly noble. I have need of someone like that. I must find someone to perform a [" .. eq.say_link("dangerous mission") .. "] or perhaps you would rather seek out the [" .. eq.say_link("Rat King") .. "] or [" .. eq.say_link("crush the undead") .. "].");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 13288})) then -- Order of Thunder

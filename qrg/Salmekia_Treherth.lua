function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Welcome to Surefall Glade, %s, the home of the Jaggedpine Treefolk. I help teach young druids the ways of our people. We have worshipers of both Karana, the Storm Lord, and Tunare, the All Mother, living here in the glade. If you are a new druid I will help you obtain a [" .. eq.say_link("suit of clothing") .. "] that will offer comfort and protection while working in the wilds and help protect you from the weapons of the Gnolls that wish to take these lands.",e.other:GetName()));
	elseif(e.message:findi("suit of clothing")) then
		e.self:Say("You must use this specially prepared Curing Kit to craft the clothing. Each article of clothing requires different materials for its construction. Do you plan on crafting Pine Druid [" .. eq.say_link("Gloves") .. "], Pine Druid [" .. eq.say_link("Boots") .. "], a Pine Druid [" .. eq.say_link("Bracer") .. "], a Pine Druid [" .. eq.say_link("Cap") .. "], Pine Druid [" .. eq.say_link("Leggings") .. "], Pine Druid [" .. eq.say_link("Sleeves") .. "], or a Pine Druid [" .. eq.say_link("Tunic") .. "]? When you have been outfitted and are ready I will tell you of a [" .. eq.say_link("task") .. "] that you can assist with.");
		e.other:SummonItem(17125);
	elseif(e.message:findi("glove")) then
		e.self:Say("To craft Pine Druid Gloves you require two [" .. eq.say_link("silk thread") .. "], ruined gnoll leather gloves, two giant field rat whiskers, and a large king snake skin. Once you have the necessary components combine them in your Curing Kit with this Tattered Glove Pattern.");
		e.other:SummonItem(19559);
	elseif(e.message:findi("boot")) then
		e.self:Say("To craft Pine Druid Boots you require two [" .. eq.say_link("silk thread") .. "], ruined gnoll leather boots, two giant field rat whiskers, and a large whiskered bat fur. Once you have the necessary components combine them in your Curing Kit with this Tattered Boot Pattern.");
		e.other:SummonItem(19561);
	elseif(e.message:findi("bracer")) then
		e.self:Say("To craft an Pine Druid Bracer you require a [" .. eq.say_link("silk thread") .. "], a ruined gnoll leather bracer, and a giant field rat whiskers. Once you have the necessary components combine them in your Curing Kit with this Tattered Wristband Pattern.");
		e.other:SummonItem(19558);
	elseif(e.message:findi("cap")) then
		e.self:Say("To craft a Pine Druid Cap you require two [" .. eq.say_link("silk thread") .. "], a ruined gnoll leather cap, a large whiskered bat fur, and a large field rat pelt. Once you have the necessary components combine them in your Curing Kit with this Tattered Cap Pattern.");
		e.other:SummonItem(19555);
	elseif(e.message:findi("legging")) then
		e.self:Say("To craft Pine Druid Leggings you require three [" .. eq.say_link("silk thread") .. "], ruined gnoll leather leggings, two large whiskered bat furs, and a giant field rat pelt. Once you have the necessary components combine them in your Curing Kit with this Tattered Pant Pattern.");
		e.other:SummonItem(19560);
	elseif(e.message:findi("sleeve")) then
		e.self:Say("To craft Pine Druid Sleeves you require two [" .. eq.say_link("silk thread") .. "], ruined gnoll leather sleeves, two large whiskered bat furs, and a giant field rat pelt. Once you have the necessary components combine them in your Curing Kit with this Tattered Sleeves Pattern.");
		e.other:SummonItem(19557);
	elseif(e.message:findi("tunic")) then
		e.self:Say("To craft a Pine Druid Tunic you require four [" .. eq.say_link("silk thread") .. "], a ruined gnoll leather tunic, a giant whiskered bat fur, and a giant field rat pelt. Once you have the necessary components combine them in your Curing Kit with this Tattered Tunic Pattern.");

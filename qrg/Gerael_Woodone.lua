function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say(string.format("It is good to meet you, %s. You, my friend, are an adventurer. The rugged look of you testifies to that. Let me know if you plan to adventure in the Plains of Karana. I have need of a person such as yourself to [" .. eq.say_link("deliver a flask") .. "].",e.other:GetName()));
	...
	elseif(e.message:findi("Unkempt Druid")) then
		e.self:Say("The Unkempt Druids are a radical splinter group of druids. Their beliefs have been contorted by the mad druid [" .. eq.say_link("Jale Phlintoes") .. "]. It is he who engineers and coordinates the druids' transgressions. From setting lumbermills aflame to murdering any man who dares to wear a bearhide. They must be stopped!! Citizens must learn to understand Tunare's will, not fear it.");
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18911})) then
		e.self:Say("Oh my!! Our Qeynos Ambassador, Gash, is in danger. Please take the note over to Captain Tillin of the Qeynos Guard then find Gash and inform him [" .. eq.say_link("they are trying to kill him") .. "]. Go!!");
		e.other:Ding();

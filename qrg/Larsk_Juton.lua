...
function event_say(e)
	if(e.message:findi("hai")) then
		e.self:Say("Hail, Adventurer! I hope you are enjoying your time in Surefall Glade. You must be careful when leaving the Jaggedpine. There have been many report of [" .. eq.say_link("Sabertooths") .. "] attacking travelers.");
	elseif(e.message:findi("Sabertooths")) then
		e.self:Say("The gnolls of Blackburrow are called Sabertooths. They have been seen in force on a regular basis. They are surely up to something. We have even heard rumors of a [" .. eq.say_link("pact") .. "] between Qeynos merchants and the dogs.");
	elseif(e.message:findi("pact")) then
		e.self:Say("Sources have come forward to tell of a pact between a merchant of Qeynos and the Sabertooths. It all has to do with blades and brew. We hear that a meeting will occur soon, somewhere in the Qeynos Hills at night. Do not fear, we shall find a brave ranger to [" .. eq.say_link("halt this meeting") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13309})) then
		e.self:Say("So, I see you rid the hills of the beast. Good work! I have a reward for you. I hope it will be usefull. I am afraid this gnoll\'s death will not halt the alliance between the two. I shall require your services to [" .. eq.say_link("assist in the extermination of the gnoll brewers") .. "].");
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 17970,item2 = 17970, item3 = 17970})) then
		...
		e.other:AddEXP(1000);
		e.other:GiveCash(math.random(5),math.random(5),math.random(5),0);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 18811})) then
		...
		e.other:Faction(135,1,0); -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,18815,200);
	end

function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Welcome to Surefall Glade friend. I am Errin Pinewhisper, Ranger of Karana the Storm Lord. You will find worshipers of both Karana and Tunare residing here in harmony. I train young rangers of both faiths and assist them in getting prepared for work in the often dangerous lands beyond the glade. If you are in need of a [" .. eq.say_link("sturdy outfit") .. "] suitable for work as a young ranger then I will gladly instruct you on the means to obtain one.");
	elseif(e.message:findi("sturdy outfit")) then
		e.self:Say("This specially prepared curing kit will allow you to craft an outfit that will protect you in the wilds of Norrath. The materials required for the outfit vary depending on the piece you desire to craft. Do you desire to craft Pine Scout [" .. eq.say_link("Gloves") .. "], Pine Scout [" .. eq.say_link("Boots") .. "], a Pine Scout [" .. eq.say_link("Bracer") .. "], a Pine Scout [" .. eq.say_link("Cap") .. "], Pine Scout [" .. eq.say_link("Leggings") .. "], Pine Scout [" .. eq.say_link("Sleeves") .. "], or a Pine Scout [" .. eq.say_link("Tunic") .. "]? Once you are properly outfitted there is an [" .. eq.say_link("issue") .. "] that you may be of assistance in resolving.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 19949})) then
		...
		e.other:Faction(267,1); --qrg protected animals
		e.other:Faction(347,-2); --unkempt druids

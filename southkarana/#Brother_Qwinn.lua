function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("In the name of the [" .. eq.say_link("Brotherhood of the Lost Circle") .. "], I greet you.");
	elseif(e.message:findi("brotherhood of the lost circle")) then
		e.self:Say("The [" .. eq.say_link("monks of the Whistling Fist") .. "] are called the Lost Circle by others. We are an ancient league of monks, long forgotten and nearly extinct. Within this realm, there are only two. [" .. eq.say_link("Brother Zephyl") .. "] and myself. We seek the items stolen from us. I seek the [" .. eq.say_link("Code of Zan Fi") .. "].");
	...
	elseif(e.message:findi("code of zan fi")) then
		e.self:Say("The tome called the Code of the Whistling Fist has been stolen from me. It contains many secrets pertaining to our brotherhood. Thankfully, it is magically locked and only a Zan Fi master could unlock it. I require a monk to retrieve it. Would you be an [" .. eq.say_link("interested monk") .. "]?");
	...
	elseif(e.message:findi("brother zephyl")) then
		e.self:Say("Brother Zephyl is searching for one who can return the [" .. eq.say_link("idol of Zan Fi") .. "]. I believe he is somewhere in the vicinity of the Rathe Mountains.");
	elseif(e.message:findi("idol of zan fi")) then
		e.self:Say("The idol of Zan Fi is a totem that is sacred to our brotherhood. I cannot share its secrets, but I can tell you that [" .. eq.say_link("Brother Zephyl") .. "] will offer to you a reward for its return..");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12316,item2 = 10114})) then -- Code of Zan Fi, Purple Headband
		...
		e.self:Say("We thank you for the return of the Code of the Whistling Fist. Take this sewing needle. You shall find it useful should you aid [" .. eq.say_link("Brother Zephyl") .. "] in his quest. His item, the needle, a swatch of shadow silk and a scroll containing Jonathan's Whistling Warsong. Into a sewing kit they will be going. And into the brotherhood will you. I hope you do not wish me to [return the headband]..");

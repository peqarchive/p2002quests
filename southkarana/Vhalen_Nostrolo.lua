...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Nice to meet you.  Do not the plains of Karana abound with life?  This land is truly blessed by the gods.  Would you like to [" .. eq.say_link("hear a tune") .. "] or will you pass up the opportunity?");
	elseif(e.message:findi("hear a tune")) then
		e.self:Say("Inside this heart, I am trapped with the sight of beautiful colors, all memories of your love. They attack me with full might. You left me here in this garden of grey Never to see the true colors of the happiness of day Here in my heart I sit and I cry. You have left me alone and my colors have all died...  What do you think? Is it a [" .. eq.say_link("happy tune") .. "] or a [" .. eq.say_link("sad tune") .. "]?");
	elseif(e.message:findi("happy tune")) then
		e.self:Say("It does need work. It should make the listener sad. I fear all my best tunes are behind me. That's why I am here. I am trying to write a [" .. eq.say_link("new composition") .. "].");
	...
	elseif(e.message:findi("sad tune")) then
		e.self:Say("It takes a broken heart to truly appreciate that tune. I'm glad you enjoyed it. My dear [" .. eq.say_link("Metala") .. "] left me one year ago. The wound has healed, but the scar remains. I am presently trying to complete a [" .. eq.say_link("new composition") .. "]. Maybe that will turn me around.");
	elseif(e.message:findi("Metala")) then
		e.self:Say("My dear sweet Metala Highflit. She was a bard I trained and fell in love with. We were perfect together. We composed many love ballads during those years. I just love the way she used to brush the hair away from her face and to see her in the moonlight brought daylight to my soul . We had such times ! I remember once, we had dinner at the Lion's Mane and we ordered a burrow pie for dessert. She got some whipped cream on her nose! Oh, it was so cute... am I [" .. eq.say_link("boring") .. "] you or should I tell you [" .. eq.say_link("where it went wrong") .. "]?");
	...
	elseif(e.message:findi("where it went wrong")) then
		e.self:Say("It was like that for years and then it all just ended. She began to hang around some man called Garuc Anehm. She started attending evening functions. She said it was choir practice for the Temple of Life. Do you want to know [" .. eq.say_link("what I found out") .. "], or am I [" .. eq.say_link("boring") .. "] you?");
	elseif(e.message:findi("what I found out")) then
		e.self:Say("Feeling neglected, I also went to join the choir. To my shock, I learned from High Priestess Jahnda that Metala was never in the choir. When I mentioned that fellow Garuc's name, she gave me a dirty look and would say nothing further.  I must be boring you. You don't want to [" .. eq.say_link("hear the end") .. "]...");	
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13114})) then -- Lisera Lute
		...
		e.other:Faction(192,2,0); -- League of Antonican Bards

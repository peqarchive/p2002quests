function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings. friend! I am Gallin Woodwind of the Emerald Warriors. It is my charge to send forth Fier'Dal to [" .. eq.say_link("hunt for meat") .. "]. Kelethin's citizens must be nourished and this requires meat. Such is the circle of life.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12121})) then
		...
		e.other:Faction(212,4); -- Merchants of Felwithe
		e.other:Faction(174,4); -- Kelethin Merchants

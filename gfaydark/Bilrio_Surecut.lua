function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, friend. If fine weapons are what you seek then you shall find them here in my shop. I have traveled far and wide for the finest weapons. Still, there are a few [" .. eq.say_link("unusual weapons") .. "] which have eluded me.");
	elseif(e.message:findi("unusual weapons")) then
		e.self:Say("Unusual weapons? Yes. There are a few which I sought. Such an item is the [" .. eq.say_link("brazen brass kilij") .. "].");
	elseif(e.message:findi("brazen brass kilij")) then
		e.self:Say("The brazen brass kilij is a legendary magical weapon designed by the ancients and long lost to the winds of time. I have searched the world for it to no avail. Perhaps the [" .. eq.say_link("legend of Husam") .. "] will add some insight to my quest. I have some knowledge of its design and I am sure I can [" .. eq.say_link("craft the kilij") .. "].");
	elseif(e.message:findi("legend of husam")) then
		e.self:Say("Legend says the kilij was forged for a great paladin for his assault on the land of the dead. He was to slay the evil siblings Dimetreas and Byzola. It is said he killed Byzola. With the kilij he was unstoppable. Dimetreas knew this and cast an [" .. eq.say_link("evil spell") .. "] upon the kilij.");

function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Are not the woods of the Faydarks a lovely sight? Tunare has truly blessed us. It is unfortunate that the orcs of [" .. eq.say_link("Crushbone Citadel") .. "] have chosen to invade our lands.");
	elseif(e.message:findi("crushbone citadel")) then
		e.self:Say("Within the Greater Faydarks can be found the entrance to Crushbone Citadel, home of the orcs. They have increased their raids on our lands. We do not know why. The High Elder of Kelethin has instructed us to keep tabs on the orcs' movements. Hmmm.. How would you like to [" .. eq.say_link("assist the scouts") .. "]?");
	elseif(e.message:findi("assist the scouts")) then
		e.self:Say("We trust this operation to high standing rogues of the Scouts of Tunare. Easy it may be, but the reward is a scout cape. The scout cape is meant for Scouts of Tunare only. Have you [" .. eq.say_link("contributed to the Scouts' cause") .. "]?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12183})) then

function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Welcome friend. you're a great leap from any sort of civilization.  What brings you to this foul place? Are you an [" .. eq.say_link("adventurer") .. "] or just a [" .. eq.say_link("connoisseur") .. "] of wretched foliage and damp clothing?");
	elseif(e.message:findi("adventurer")) then
		e.self:Say("Well. I gathered as much. but I try to avoid assumptions.  Are you by chance heading [" .. eq.say_link("to the temple") .. "]?  It seems to be quite a draw for adventurers and academics alike.  Though we've yet to witness either group leave the structure once they've entered it.");
	elseif(e.message:findi("to the temple")) then
		e.self:Say("Well. take care and prepare for the most horrific of possibilities. my friend.  Our trip has been terrifying enough and we've yet to enter the [" .. eq.say_link("temple") .. "] itself.  I've seen enough from here to confirm some of the [" .. eq.say_link("rumors") .. "] that we were sent to investigate.");
	elseif(e.message:findi("rumors")) then
		e.self:Say("The Academy sent my colleagues and I to investigate several reports about Tae Ew tribal activity that have found their way to Freeport during the course of the last lunar cycle.  The rumors stated that the primitive creatures have made several great discoveries of a necromantic nature in an incredibly short time.  I was assigned to gather any artifacts and information that I could uncover.  If you should learn more about their [" .. eq.say_link("magic") .. "]. please return to me before we leave and you will be rewarded.");

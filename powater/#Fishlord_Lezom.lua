...
function event_trade(e)
	...
	elseif (not started) and (item_lib.check_turn_in(e.self, e.trade, {platinum = 100})) then 
		...
		started = true;
		eq.set_timer("wave_timer", 30 * 1000);	--initial timer shorter
		eq.set_timer("FinalNamed", final_boss_timer * 1000);	--6 hrs?? lol
		eq.start(209);
	end
...
function spawn_mob(npc_id,loc)	--loc also determines grid
	local spawn_locs = { [1] = {128, 2, -352, 0}, [2] = {168, 76, -362, 375}, [3] = {154,164,-362,347}, [4] = {95,281,-362,375}, [5] = {22,275,-362,129}, [6] = {38,403,-365,226}, [7] = {128,33,-362,19}, [8] = {87,337,-362,267} };
	local grid = {210,211,212,213,214,217,215,216}	--Fishlord_Lezom area
...
function event_timer(e)
	if(e.timer == "wave_timer") then
		...
		--eq.zone_emote(14,"Wave count: [" .. wave .. "]  Random Roll [" .. rand .. "/100] Spawn Chance: [" .. miniNamedChance .. "%]");
	...
	elseif(e.timer == "FinalNamed") then
		...
		eq.update_spawn_timer(369034,21600 * 1000);	--Set respawn timer to 6 hrs
		eq.depop_with_timer();
	end
end

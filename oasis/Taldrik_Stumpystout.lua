...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13943, item2 = 13036, item3 = 13035})) then
		e.self:Say("Ah ha! So ye are Bronlor's chosen aye? Well den these fine brews can only mean one thing! Yep its dat youre a drinker like meh! Arg, if I only had me [" .. eq.say_link("recipe") .. "].");
		e.other:Ding();
		e.other:AddEXP(100);

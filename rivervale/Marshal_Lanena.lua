...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say("Hello. " .. e.other:GetName() .. ".  I am Lanena Wickystick. marshal of all Vale concerns.  If there are any troubles brewing in our fine town which concern the Guardians of the Vale. please inform me.  You must be a [" .. eq.say_link("new deputy") .. "] or are you an [" .. eq.say_link("outsider") .. "]?");
	elseif(e.message:findi("new deputy") and fac > 2 and fac < 5) then
		e.self:Say("It is good to see such fine stock in the ranks of the Guardians.  Being new. there is much to learn. in battle and in life itself.  If you are not presently obligated. we have need of you here in the hollow.  There seems to be a [" .. eq.say_link("small problem") .. "].");
	...
	elseif(e.message:findi("rantho rapier")) then
		e.self:Say("The Rantho Rapier was crafted by the great blacksmith Rantho Goobler.  It was designed for use by the warriors of the wee folk.  With its light weight and special two-hand hilt, it becomes a formidable weapon in the hands of our younger deputies.  Only a [" .. eq.say_link("new deputy") .. "] has the right to earn one.");
	end
end

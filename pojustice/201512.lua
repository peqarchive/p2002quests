--#a_sentenced_prisoner (201512)  -- Wood Elf Female
--Trial of Hanging
--pojustice
...
function event_timer(e)	--Total timer before prisoner dies = 42 seconds
	if e.timer == "noose" then
		stage = stage + 1;
		if stage == 1 then
			eq.local_emote({e.self:GetX(), e.self:GetY(), e.self:GetZ()},7,150,"The prisoner clutches at her throat, trying desperately to breathe.");
		elseif stage == 2 then
			e.self:SetAppearance(3);
			eq.local_emote({e.self:GetX(), e.self:GetY(), e.self:GetZ()},7,150,"The prisoner falls to the ground, legs kicking and fingers clawing at her throat.");
		elseif stage == 3 then
			eq.local_emote({e.self:GetX(), e.self:GetY(), e.self:GetZ()},7,150,"The prisoner gives one final twitch and suddenly becomes still, her limbs no longer flailing.  You have failed.");

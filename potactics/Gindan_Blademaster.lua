...
function event_hp(e)
	if e.hp_event == 61 and e.self:GetSpawnPointID() ~= 0 then
		...
		if rand > 10 then
			for n = 1,2 do
				splitter = eq.spawn2(214031,0,0,e.self:GetX() + math.random(-5,5), e.self:GetY() + math.random(-5,5),e.self:GetZ(),e.self:GetHeading());	--Gindan_Blademaster (214031)

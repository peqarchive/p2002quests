...
--Raid Instance Gozer - Saryrn's Tower
--potorment

local show_debug = false;
local disabled_spawns = {368961,346755,346871,347038,347039,347040,347041,347042,347043,347044,347045,347046,347047};	--spawns in the allowable grid that should not be killable/usable 
...
function event_say(e)
    ...
    elseif(e.message:findi('brave')) then
        ...
        local zoneGlobal = "POTORMENT-" .. tostring(e.other:GuildID());
        ...
        if (instanceId ~= nil) then
            ...
            e.other:MovePCInstance(207, tonumber(instanceId), -8,-8,-55,0); 
        else
...
function event_spawn(e)
  ...
  if (show_debug) then
    ...
    if (instance_id ~= 0) then
		local table_locs = {	[1] = {max_x = 150, min_x = -150, max_y = 75, min_y = -250},
								[2] = {max_x = 0, min_x = -450, max_y = 2000, min_y = 1500},
								[3] = {max_x = 0, min_x = -400, max_y = 1200, min_y = 300}
							};
...
function scan_for_out_of_prox()
	...
	for k,spawns in pairs(disabled_spawns) do
	...
	if (player_list ~= nil) then
		for player in player_list.entries do
			...
			if (moveMe) then
				...
				player:MovePCInstance(207, tonumber(instanceId),-340,1705,-490,400); 
			end
		end
	end

...
--Raid Instance Gozer - PoI Factory
--poinnovation

local show_debug = false;
local disabled_npcs = {206044,206045,206053,206054,206055};	--npcs in the allowable grid that should not be killable/usable 
...
function event_say(e)
    ...
    elseif(e.message:findi('brave')) then
        ...
        if (instanceId ~= nil) then
            ...
            e.other:MovePCInstance(206, tonumber(instanceId), 51, 67, 8, 256); 
        else
...
function event_spawn(e)
  ...
  if (show_debug) then
    ...
    if (instance_id ~= 0) then
		local table_locs = {	[1] = {max_x = 1700, min_x = 600,  max_y = 700, min_y = -700},
								[2] = {max_x = 600, min_x = -450, max_y = 150, min_y = -600},
								[3] = {max_x = 400, min_x = 175, max_y = 600, min_y = 400} };
	...
	if (player_list ~= nil) then
		for player in player_list.entries do
			...
			if (moveMe) then
				...
				player:MovePCInstance(206, tonumber(instanceId), 270, 525, -45, 211); 
			end
		end
	end

function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Mez got secretz!'  Kragia's eyes glint with menace.  'Mez bet yoo wantz to know my [" .. eq.say_link("secretz") .. "]!");
	elseif(e.message:findi("secretz")) then
		e.self:Say("The Dark Stranger!  He show me how to make some...' Kragia cackles. '...some fingz for hurti and bashin!'  Kragia grins. and brandishes a long sword that crackles with an energy of its own.  'Yoo need to [" .. eq.say_link("work") .. "] dem weaponz. and den [" .. eq.say_link("finish") .. "] dem!");
	elseif(e.message:findi("work")) then
		e.self:Say("Trollz. we'z always gud at recycling scrap!  Take your [" .. eq.say_link("old weapon") .. "] and den use a smithy hammer to bash in two sheetz of dem magic [" .. eq.say_link("acrylia ore") .. "]. wiv a special [" .. eq.say_link("mold") .. "]!  Yooz have to use da special Grobb forge.");
	elseif(e.message:findi("acrylia ore")) then
		e.self:Say("He show me some real gud metal. he sayz it come from far away.  It look funny to me!  He tellz me dat a block of it. and a smithy hammer and water. can make dem sheet.  Den. wiv dat sheet. yoo use yer hammer again but wiv [" .. eq.say_link("magical temper") .. "] dis time.");
	...
	elseif(e.message:findi("finish")) then
		e.self:Say("Dis da fun bit!  In da special forge. yooz put in da worked weapon and use chisel and file. wiv some [" .. eq.say_link("bile temper") .. "]!  Yooz make weaponz as gud as Kragia!'  Kragia leans over to you and whispers loudly. 'Yooz can also make dem magical wiv imbued shinystonez.  Dey really gud!");

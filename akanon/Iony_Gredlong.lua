...
function event_enter(e)
	if(e.other:HasItem(18775) == true) then -- A tattered note
...
function event_say(e)
	...
	elseif(e.message:findi("cleric")) then
		e.self:Say("Well you would certainly be eligible for my training then young " .. e.other:GetName() .. ". I have a number of armor recipes that I give to young clerics to make sure that when they leave the comfort of the Steamfont Mountains they are ready for whatever battles they may face. If you are [" .. eq.say_link("interested in creating") .. "] your own armor then all you must do is simply ask and I will give you instructions to get started.");
	elseif(e.message:findi("interested in creating")) then
		...
		e.other:SummonItem(17253); -- Holy Assembly Kit
	elseif(e.message:findi("various items")) then
		e.self:Say("There are numerous items that you will need to collect and combine in your kit. I will present you for the recipe for Shortnoble Platemail [" .. eq.say_link("Helms") .. "], [" .. eq.say_link("Bracers") .. "], [" .. eq.say_link("Armguards") .. "], [" .. eq.say_link("Boots") .. "], [" .. eq.say_link("Greaves") .. "], [" .. eq.say_link("Gauntlets") .. "] and [" .. eq.say_link("Breastplates") .. "]. When you are ready to attempt a specific piece please let me know what piece you want to craft and I will give you the necessary mold along with the recipe.");
	...
	elseif(e.message:findi("final favor")) then
		e.self:Say("I thank you for returning to me young Gearsword. I must say that it is now quite clear that you are very capable of completing any task I should assign you due to your eagerness to learn. I am currently in need of a few specific items to craft a weapon worthy of any servant of Brell. It is my hope that I can pass these out to our new recruits quite soon. Will you [" .. eq.say_link("collect the items") .. "] I require?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 9105,item2 = 9105,item3 = 9105,item4 = 9106})) then -- 3x Flawless Harpy Claw and Ebon Drake Backbone
		e.self:Say("Thank you! Here, take this staff and good luck on your journey."); -- Text made up
		e.other:QuestReward(e.self,0,0,0,0,9107,100); -- Walking Staff of the Shortnoble
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 18775})) then -- A tattered note

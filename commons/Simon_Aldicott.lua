...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 6708})) then -- A Gem Encrusted Casket
		...
		eq.spawn2(21031,0,0,2901,259.7,-51,50); -- a tortured revenant
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 6709})) then -- Revenant Ash
		...
		e.other:QuestReward(e.self,0,0,0,0,15693,20000); -- Spell: Divine Might
	end

...
--Raid Instance Gozer - Grummus Castle Location
--podisease

local show_debug = false;
local disabled_spawns = {344900,344767};	--spawns in the allowable grid that should not be killable/usable 
...
function event_say(e)
    ...
    elseif(e.message:findi('brave')) then
        ...
        local zoneGlobal = "PODISEASE-" .. tostring(e.other:GuildID());
        ...
        if (instanceId ~= nil) then
            ...
            e.other:MovePCInstance(205, tonumber(instanceId), 2175,355,375,384); 
        else
...
function event_spawn(e)
  ...
  if (show_debug) then
    ...
    if (instance_id ~= 0) then
		local table_locs = {	[1] = {max_x = 2300, min_x = 1650, max_y = 425, min_y = 150},
								[2] = {max_x = 2300, min_x = 1750, max_y = 150, min_y = -400},
								[3] = {max_x = -1600, min_x = -2050, max_y = -1050, min_y = -1300}
							};
	...
	if (player_list ~= nil) then
		for player in player_list.entries do
			...
			if (moveMe) then
				...
				player:MovePCInstance(205, tonumber(instanceId), -1750,-1245,-55,0); 
			end
		end
	end

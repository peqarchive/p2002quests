--Askr_the_Lost (210062)
--Zone:postorms

function event_say(e)
	local qglobals = eq.get_qglobals(e.self, e.other);
	
	if e.message:findi("Hail") and tonumber(qglobals.pop_pos_askr_the_lost)	== nil then
		e.self:Emote("looks up at you, and when he does, you take note of the scraggly beard on his weather-worn face; the scent of ale permeates your senses and pushes you to look away slightly. In an apparent drunken stupor, Askr begins to speak with surprising clarity. 'Here to wash the windows and clean the flooring, are you? No? Well, you can't be much help then. And the likes of you obviously wouldn't be able to get rid of the massive [" .. eq.say_link("what problem?",false,"problem") .. "] we're having. Bah, says I. You can help me no more than any of these other vagrants. Leave me alone to my ale and my misery.'");
	elseif e.message:findi("what problem") and tonumber(qglobals.pop_pos_askr_the_lost)	== nil then
		e.self:Emote("points drunkenly towards the exit of the cave. 'Have you not seen the foul denizens of destruction outside? Hrmph! Going everywhere they please, pillaging, plundering. . . I'm lucky to have survived this long. Bah, it doesn't matter, there's nothing that anyone can do to stop them, and that is why I'm still stuck here with my *hic* potions.'");
	elseif e.message:findi("Hail") and tonumber(qglobals.pop_pos_askr_the_lost_final) == 1 then
		e.self:Say("You have returned to me, but for what purpose? You already have the way to the Bastion of Thunder, so please make haste there and rid the Bastion of those horrid storm giants! Only then may the balance that once ruled over these thunderous lands return to its rightful place.")
	elseif tonumber(qglobals.pop_pos_askr_the_lost) == 2 then
		...
		elseif e.message:findi("Hail") then
			e.self:Say("I was afraid you would not return to finish the noble story you had started! But now I see my worries were ill placed. You have proven yourself a worthy adversary to even the mightiest of foes, but your journey does not end here! Seek me out that which I require and let your story [" .. eq.say_link("continue") .. "] on its way to greatness!");
		end
	elseif tonumber(qglobals.pop_pos_askr_the_lost) == 1 then	--checks for giant head handin
		if e.message:findi("It was me") then
			e.self:Say("Truly, I am amazed that someone of your caliber was able to do what I could not. I have been stuck in these desolate caves for so long, I had almost lost hope that I would ever find a way to get rid of those accursed giants. But you. . . you have been able to best at least one of them. It is reasonable to suggest that if you are able to dispatch one, you may be able to dispatch others. I need to know for sure if you are as capable an individual as it appears you are. This is very important, so are you [" .. eq.say_link("i am paying attention",false,"paying attention") .. "?]");
		elseif e.message:findi("I am paying attention") then
			e.self:Say("This place was once beautiful and serene, with the great Karana's showers falling day and night soothing the lands and the creatures that inhabit it. Alas, there was an invasion of a superior force -- the storm giants which you have encountered. They laid waste to the lands, burned down trees, and have caused so much havoc I don't even know what is to become of it. Originally, they came as one group and crossed the breadth of the land pillaging and plundering all that they could find. Over time, though, they grew apart and distinct factions grew around Mount Grenidor. [" .. eq.say_link("continue") .. "]");
		elseif e.message:findi("continue") then
			e.self:Say("To the south of Grenidor lay Srerendi, Shores of the Lost and the Srerendi storm giants. Born of the ocean and the cool air, they lay claim to the shores and the plains inland as their own. To the west of Grenidor lay Krendic, Sands of Chaos and the Krendic storm giants. They have skin that is thick as dirt and blend in well with their surroundings. To the north of the great mount lay Kelek`Vor, the Forest of Tears and the Kelek`Vor storm giants. They are born of the wood and take great pleasure in seeing it used to their own purposes. [" .. eq.say_link("continue") .. "]");
			eq.set_global("pop_pos_askr_the_lost", "2", 5, "F");	--used to differentiate between the two [continue] statements
		end
	elseif tonumber(qglobals.pop_pos_askr_the_lost) == 3 then
		if e.message:findi("Bastion of Thunder") then
			...
			e.other:SummonItem(17192);		--Askr's Bag of Verity
		elseif e.message:findi("hail") then
			e.self:Say("A hearty welcome back to you, friend. You have the means to push back the scourge of giants... you must go forth to the [" .. eq.say_link("Bastion of Thunder") .. "] and finish there what you have started here. It is imperative that you clear the forces of giants from the Bastion so that order can be restored to the thunderous reaches beyond.");
		end
	end
end
...
function event_trade(e)
	...
	if tonumber(qglobals.pop_poj_valor_storms) == 1 then
		if item_lib.check_turn_in(e.self, e.trade, {item1 = 28749}) or item_lib.check_turn_in(e.self, e.trade, {item1 = 28781}) or item_lib.check_turn_in(e.self, e.trade, {item1 = 28782}) then  
			e.other:Message(7,"The drunken stupor that Askr had been in earlier appears to vanish in an instant; a glimmer of hope now flashes in the depths of his eyes. 'This is the head of a giant. . . a storm giant from outside the caves.' Askr ponders what this revelation could mean. He mutters to himself, 'If it is possible that someone has indeed bested a giant here in the planes, then perhaps it is possible. . .' He looks back at you very sternly and asks, 'Tell me now and be truthful, was it [" .. eq.say_link("it was me", false, "you") .. "] who severed the head of this giant from his body?'");
			eq.set_global("pop_pos_askr_the_lost", "1", 5, "F");	--handed in giant head
		elseif tonumber(qglobals.pop_pos_askr_the_lost)	== 3 and item_lib.check_turn_in(e.self, e.trade, {item1 = 11488}) then	--Esoteric Meld
		...
		elseif tonumber(qglobals.pop_pos_askr_the_lost)	== 2 and item_lib.check_turn_in(e.self, e.trade, {item1 = 11487}) then	--Askrs Sealed Bag of Verity

...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hail to you, friend.  Seek you [" .. eq.say_link("knowledge") .. "] or [" .. eq.say_link("atonement") .. "] in this [" .. eq.say_link("temple of peace") .. "]?");
	...
	elseif(e.message:findi("atonement")) then
	...
	elseif(e.message:findi("temple of peace")) then
        e.self:Say("Dedicated to the god of peace, Quellious, is this temple. It is here that her clerics and [" .. eq.say_link("paladins") .. "] pray and receive inspiration.");
	elseif(e.message:findi("paladins")) then
        e.self:Say("As the clerics are the warm hand of healing, the paladins are the stalwart shield of protection, and in times of grave need the sharp sword of justice. To our post must we always [" .. eq.say_link("remain vigilant") .. "], else dishonor might befall us.");
	elseif(e.message:findi("remain vigilant")) then
        e.self:Say("To a paladin the upholding of his duty, or honor to his temple is foremost. To fail in his duty to stand at his assigned post in time of need is a great disgrace. Few are the paladins who have done so, and their spirits have been [" .. eq.say_link("outcast") .. "] from the god of peace.");
	elseif(e.message:findi("outcast")) then
        e.self:Say("Harsh though it seems, a true paladin must always remain vigilant and dedicated to his cause. If he does not then he might find himself [" .. eq.say_link("fallen from grace") .. "].");
	elseif(e.message:findi("fallen from grace")) then
        e.self:Say("here are a few who have done so. Only a few that come to mind. Such [" .. eq.say_link("stories") .. "] are useful to remember as reminders of our beliefs.");
	elseif(e.message:findi("stories")) then
        e.self:Say("One is the story of the paladin who fell due to his weakness in the duty of his guard post. He let himself be seduced by a [" .. eq.say_link("peasant woman") .. "]");

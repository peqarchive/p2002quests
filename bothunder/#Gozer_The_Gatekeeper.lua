...
--Raid Instance Gozer - BoT Courtyard
--povalor

local show_debug = false;
local disabled_spawns = {360600,360270,360429,360448,360292,360468,360477};	--spawns in the allowable grid that should not be killable/usable 
...
function event_say(e)
    ...
    elseif(e.message:findi('brave')) then
        ...
        local zoneGlobal = "BOTHUNDER-" .. tostring(e.other:GuildID());
        ...
        if (instanceId ~= nil) then
            ...
            e.other:MovePCInstance(209, tonumber(instanceId), 150,-60,-635,0); 
        else
...
function event_spawn(e)
  ...
  if (show_debug) then
    ...
    if (instance_id ~= 0) then
		local table_locs = {	[1] = {max_x = -250, min_x = -1250, max_y = -1500, min_y = -2000},
								[2] = {max_x = 300, min_x = 75, max_y = 125, min_y = -100},
								[3] = {max_x = 275, min_x = 100, max_y = 300, min_y = 125}
							};
	...
	if (player_list ~= nil) then
		for player in player_list.entries do
			...
			if (moveMe) then
				...
				player:MovePCInstance(209, tonumber(instanceId),180,205,-1620,256);				
			end
		end
	end
